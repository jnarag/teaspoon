...
public class TreeBiPartitionInfo {
    class BiPartiotionInfo {
        BiPartiotionInfo(jebl.evolution.graphs.Node n) {
            this.n = n;
        }
    ...
    final RootedTree t;
    final int        nTips;
    HashMap<jebl.util.FixedBitSet, BiPartiotionInfo> all;

    public TreeBiPartitionInfo(RootedTree t, List<Taxon> taxa) {
        this.t = t;
        this.taxa = taxa;
        nTips = t.getExternalNodes().size();
        all = new LinkedHashMap<jebl.util.FixedBitSet, BiPartiotionInfo>();
...
//
    ...
    private jebl.util.FixedBitSet forNode(jebl.evolution.graphs.Node n) {
        final jebl.util.FixedBitSet p = new jebl.util.FixedBitSet(nTips);
        if( t.isExternal(n) ) {
            final int pos = taxa.indexOf(t.getTaxon(n));
            p.set(pos);
            ...
            for( jebl.evolution.graphs.Node c : t.getChildren(n) ) {
                final jebl.util.FixedBitSet info = forNode(c);
                p.union(info);
            }
        }
    ...
    public static double distance(TreeBiPartitionInfo t1, TreeBiPartitionInfo t2, DistanceNorm norm) {
        ...
        for( Map.Entry<jebl.util.FixedBitSet, BiPartiotionInfo> k : t1.all.entrySet() ) {
