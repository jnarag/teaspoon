 ...
 *
 * @version $Id: NexusImporter.java 935 2008-07-22 16:52:04Z rambaut $
 *
 * @author Andrew Rambaut
 * @author Alexei Drummond
 */
public class NexusImporter implements AlignmentImporter, jebl.evolution.io.SequenceImporter, jebl.evolution.io.TreeImporter, DistanceMatrixImporter {
     ...
     * @see {@link NexusImporter.NexusBlock}
     */
	 ...
	 */
	public List<jebl.evolution.sequences.Sequence> parseCharactersBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
	 ...
	 */
	public List<jebl.evolution.sequences.Sequence> parseDataBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
	 ...
	 */
    public jebl.evolution.distances.DistanceMatrix parseDistancesBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
	...
	public List<jebl.evolution.alignments.Alignment> importAlignments() throws IOException, ImportException
	{
		...
		List<jebl.evolution.alignments.Alignment> alignments = new ArrayList<jebl.evolution.alignments.Alignment>();
		...
		while (!done) {
			try {
				...
				if (block == NexusBlock.TAXA) {
					...
					List<jebl.evolution.sequences.Sequence> sequences = readCharactersBlock(taxonList);
					alignments.add(new jebl.evolution.alignments.BasicAlignment(sequences));

				} else if (block == NexusBlock.DATA) {

					// A data block doesn't need a taxon block before it
					// but if one exists then it will use it.
					List<jebl.evolution.sequences.Sequence> sequences = readDataBlock(taxonList);
					alignments.add(new jebl.evolution.alignments.BasicAlignment(sequences));
	...
	public List<jebl.evolution.sequences.Sequence> importSequences() throws IOException, ImportException {
		...
		List<Taxon> taxonList = null;
		List<jebl.evolution.sequences.Sequence> sequences = null;
	 ...
	 */
	public List<jebl.evolution.distances.DistanceMatrix> importDistanceMatrices() throws IOException, ImportException {
		boolean done = false;
		List<Taxon> taxonList = null;
		List<jebl.evolution.distances.DistanceMatrix> distanceMatrices = new ArrayList<jebl.evolution.distances.DistanceMatrix>();
		while (!done) {
			try {
				NexusBlock block = findNextBlock();
				if (block == NexusBlock.TAXA) {
					...
					jebl.evolution.distances.DistanceMatrix distanceMatrix = readDistancesBlock(taxonList);
	 ...
	 */
	private List<jebl.evolution.sequences.Sequence> readSequenceData(List<Taxon> taxonList) throws ImportException, IOException
	{
		...
		if (isInterleaved) {
			...
			for (int k = 0; k < taxonCount; k++) {
				jebl.evolution.sequences.Sequence sequence = new BasicSequence(sequenceType, taxList.get(k), sequencesData.get(k));
			...
			for (int i = 0; i < taxonCount; i++) {
				...
				jebl.evolution.sequences.Sequence sequence = new BasicSequence(sequenceType, taxon, seqString);
				sequences.add(sequence);
			}
     ...
     */
    static void parseAndClearMetaComments(jebl.util.Attributable item, jebl.evolution.io.ImportHelper importHelper) throws ImportException.BadFormatException {
	 ...
	 */
	private List<jebl.evolution.sequences.Sequence> readCharactersBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
		...
		List<jebl.evolution.sequences.Sequence> sequences = readSequenceData(taxonList);
	 ...
	 * Reads a 'DATA' block.
	 */
	private List<jebl.evolution.sequences.Sequence> readDataBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
	 ...
	 */
	private jebl.evolution.distances.DistanceMatrix readDistancesBlock(List<Taxon> taxonList) throws ImportException, IOException
	{
		if (taxonList == null) {
	...
	private List<Tree> readTreesBlock(List<Taxon> taxonList) throws ImportException, IOException {
        ...
        while( true ) {
			jebl.evolution.trees.RootedTree tree = readNextTree(lastToken);
			if (tree == null) {
				break;
			}
			trees.add(tree);
	...
	private jebl.evolution.trees.RootedTree readNextTree(String[] lastToken) throws ImportException, IOException
	{
		try {
			jebl.evolution.trees.SimpleRootedTree tree = null;
            ...
            if ( isUnrooted || token.equalsIgnoreCase("TREE")) {
				...
				try {
					...
					tree = new jebl.evolution.trees.SimpleRootedTree();
			...
			return compactTrees ? new jebl.evolution.trees.CompactRootedTree(tree) : tree;
	...
	private jebl.evolution.graphs.Node readInternalNode(jebl.evolution.trees.SimpleRootedTree tree) throws IOException, ImportException
	{
		...
		jebl.evolution.graphs.Node node = tree.createInternalNode(children);
	 ...
	 */
	private jebl.evolution.graphs.Node readExternalNode(jebl.evolution.trees.SimpleRootedTree tree) throws ImportException, IOException
	{
        ...
        try {
            final jebl.evolution.graphs.Node node = tree.createExternalNode(taxon);
	...
	static void parseMetaCommentPairs(String meta, jebl.util.Attributable item) throws ImportException.BadFormatException {
	...
	protected final jebl.evolution.io.ImportHelper helper;
}