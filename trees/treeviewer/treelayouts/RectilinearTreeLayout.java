...
public class RectilinearTreeLayout extends AbstractTreeLayout {
    ...
    public Shape getCollapsedNode(jebl.evolution.graphs.Node node, double ratio) {
        jebl.evolution.graphs.Node first = node;
        while( ! tree.isExternal(first) ) {
            first = tree.getChildren(first).get(0);
        }
        jebl.evolution.graphs.Node last = node;
        while( ! tree.isExternal(last) ) {
            final List<jebl.evolution.graphs.Node> children = tree.getChildren(last);
    ...
    public int getNodeMarkerRadiusUpperLimit(jebl.evolution.graphs.Node node, AffineTransform transform) {
        //final Node parent = tree.getParent(node);
        double lim = Double.MAX_VALUE;
        final Point2D n = getNodePoint(node);
        for( jebl.evolution.graphs.Node a : tree.getAdjacencies(node) ) {
            //final Point2D n = getNodePoint(node);
        ...
        final List<jebl.evolution.graphs.Node> nodes = tree.getChildren(node);
        ...
        if( nNodes >= 2 ) {
            ...
            final jebl.evolution.graphs.Node[] rnode = {Utils.rightNb(tree, node), Utils.leftNb(tree, node)};
...
//                      ((rnode[1] == null) ? " - " :  tree.getTaxon(rnode[1]).getName()) );
            ...
            for( jebl.evolution.graphs.Node nb : rnode ) {
                if( nb != null ) {
    ...
    protected void validate() {
        ...
        final jebl.evolution.graphs.Node root = this.tree.getRootNode();
    ...
    private Point2D constructNode(jebl.evolution.graphs.Node node, double xPosition) {
        ...
        if (!tree.isExternal(node)) {
            ...
            for( final jebl.evolution.graphs.Node child : children ) {
    ...
    public boolean smallSubTree(jebl.evolution.graphs.Node node, AffineTransform transform) {
        int th = 7;
        final List<jebl.evolution.graphs.Node> children = tree.getChildren(node);
        if( children.size() < 2 ) return false;

        final jebl.evolution.graphs.Node[] ch = {children.get(0), children.get(1)};
...
//        }
    ...
    private void getMaxXPosition(final jebl.evolution.graphs.Node node, final double xPosition) {
