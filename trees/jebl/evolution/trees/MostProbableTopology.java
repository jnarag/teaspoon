...
public class MostProbableTopology {
     ...
     * Set of input trees
     */
    final List<jebl.evolution.trees.Tree> trees;
     ...
     * @param trees
     */
    public MostProbableTopology(Collection<? extends jebl.evolution.trees.Tree> trees) {
        this.trees = new ArrayList<jebl.evolution.trees.Tree>(trees);
        jebl.evolution.trees.Tree tree0 = this.trees.get(0);
    ...
    private interface NodeCallback {
         ...
         */
        void visit(jebl.evolution.graphs.Node node, jebl.util.FixedBitSet tipSet);
    }
    ...
    private interface EdgeCallback {
         ...
         */
        void visit(jebl.evolution.graphs.Edge edge, jebl.util.FixedBitSet tipSet);
    }
    ...
    private interface Info {
        // Get the tree
        jebl.evolution.trees.Tree getTree();
    ...
    private class TraversableTree {
        // the tree
        final jebl.evolution.trees.Tree t;

        TraversableTree(jebl.evolution.trees.Tree t) {
            this.t = t;
        }

        // Traberse the tree, calling call.visit for every edge.
        void traverse(EdgeCallback call) {
            final jebl.evolution.graphs.Node tip = t.getExternalNodes().iterator().next();
            traverse(call, t.getAdjacencies(tip).get(0), tip );
        }

        // Traverse all edges in partition containing n when edge (n, root) is removed.

        private jebl.util.FixedBitSet traverse(EdgeCallback call, jebl.evolution.graphs.Node n, jebl.evolution.graphs.Node root) {
            final jebl.util.FixedBitSet tipSet = new jebl.util.FixedBitSet(taxa.size());

            if( t.isExternal(n) ) {
                tipSet.set( taxa.indexOf( t.getTaxon(n) ) );
            } else {
                for( jebl.evolution.graphs.Node c : t.getAdjacencies(n) ) {
                    if( c == root ) continue;
                    final jebl.util.FixedBitSet cTips = traverse(call, c, n);
                    tipSet.union(cTips);
                }
            }
            ...
            if( needComplement ) {
                final jebl.util.FixedBitSet b = new jebl.util.FixedBitSet(tipSet);
                b.complement();
                return b;
            }
            return tipSet;
    ...
    private class UnrootedTreeInfo extends TraversableTree implements Info {
        public jebl.evolution.trees.Tree getTree() {
            return t;
        }
        ...
        class EdgeInfo extends ConditionalData {
            jebl.evolution.graphs.Edge e;

            public EdgeInfo(jebl.evolution.graphs.Edge e) {
                super();
                this.e = e;
            }
        }
        ...
        final public Map<jebl.util.FixedBitSet, EdgeInfo> m;

        public UnrootedTreeInfo(SimpleTree t) {
            super(t);
            m = new LinkedHashMap<jebl.util.FixedBitSet, EdgeInfo>();
            traverse(new EdgeCallback() {
                public void visit(jebl.evolution.graphs.Edge e, jebl.util.FixedBitSet tipSet) {
    ...
    class TraversableRootedTree {
        ...
        private jebl.util.FixedBitSet traverse(NodeCallback call, jebl.evolution.graphs.Node n) {
            final jebl.util.FixedBitSet tipSet = new jebl.util.FixedBitSet(taxa.size());

            if( t.isExternal(n) ) {
                tipSet.set( taxa.indexOf( t.getTaxon(n) ) );
            } else {
                for( jebl.evolution.graphs.Node c : t.getChildren(n) ) {
                    final jebl.util.FixedBitSet cTips = traverse(call, c);
                    tipSet.union(cTips);
                }
            }
            call.visit(n, tipSet);
    ...
    private class TreeInfo extends TraversableRootedTree implements Info {
        ...
        class NodeInfo extends ConditionalData {
            jebl.evolution.graphs.Node n;

            public NodeInfo(jebl.evolution.graphs.Node n) {
                super();
                this.n = n;
            }
        }
        ...
        final public Map<jebl.util.FixedBitSet, NodeInfo> m;

        TreeInfo(SimpleRootedTree t) {
            super(t);
            m = new LinkedHashMap<jebl.util.FixedBitSet, NodeInfo>();
            traverse(new NodeCallback() {
                public void visit(jebl.evolution.graphs.Node n, jebl.util.FixedBitSet tipSet) {
        ...
        public void setBranches() {
            traverse(new NodeCallback() {
                public void visit(jebl.evolution.graphs.Node n, jebl.util.FixedBitSet tipSet) {
                    ...
                    for( jebl.evolution.graphs.Node c : t.getChildren(info.n) ) {
     ...
     */
    public List<jebl.evolution.trees.Tree> get(final int max, final double threshold) {
        ...
        for(int nTree = 0; nTree < nTrees; ++ nTree) {
            final jebl.evolution.trees.Tree t = trees.get(nTree);
        ...
        while (queue.peek() != null && candidates.size() <= th && !(max > 0 && candidates.size() >= max) ) {
            ...
            // make a copy
            final jebl.evolution.trees.Tree tree = trees.get(info.representativeIndex);
            ...
            final jebl.evolution.trees.Tree tree1 = candidate.getTree();
            tree1.setAttribute("Frequency", (100.0 * info.count) / nTrees);
            tree1.setAttribute(jebl.evolution.io.NexusExporter.treeNameAttributeKey, "topology_" + candidates.size());
        }
        ...
        List<jebl.evolution.trees.Tree> results = new ArrayList<jebl.evolution.trees.Tree>();
    ...
    private String standardTopologyRepresentation(jebl.evolution.trees.Tree t) {
        ...
        final List<jebl.evolution.graphs.Node> adj = t.getAdjacencies(t.getNode(taxa.get(0)));           assert( adj.size() == 1 );
        final RootedTree r = new jebl.evolution.trees.RootedFromUnrooted(t, adj.get(0), true);
        return standardTop(r, r.getRootNode());
    }

    // "Standard" string representation of a rooted tree
    // Newick for taxa only, where children are sorted.
    private String standardTop(RootedTree t, jebl.evolution.graphs.Node n) {
        if( t.isExternal(n) ) {
            return Integer.toString(taxa.indexOf( t.getTaxon(n) ));
        }
        final List<jebl.evolution.graphs.Node> dec = t.getChildren(n);
