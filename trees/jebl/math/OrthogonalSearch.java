...
public class OrthogonalSearch extends MultivariateMinimum
{
	...
	// Private stuff
	//
	private jebl.math.OrderEnumerator.OEFactory orthogonalOrderingFactory_;
	...
	public OrthogonalSearch() {
		...
		this(jebl.math.OrderEnumerator.Utils.getOrderedFactory());
	}
	/**
	 * Initialization
	...
	public OrthogonalSearch(boolean shuffle) {
		//this(OrderUtils.getBiasAlternatingFactory( OrderUtils.getOrderedFactory(), OrderUtils.getZeroFactory()));
		this(shuffle? jebl.math.OrderEnumerator.Utils.getShuffledFactory() : jebl.math.OrderEnumerator.Utils.getOrderedFactory());
	}
	/**
	 * Initialization
	 */
	public OrthogonalSearch(jebl.math.OrderEnumerator.OEFactory orderingFactory) {
	...
	protected RoundOptimiser generateOrthogonalRoundOptimiser(MultivariateFunction mf) {
		jebl.math.OrthogonalHints hints = mf.getOrthogonalHints();
		if(hints!=null) {
	...
	private class OrthogonalDirection implements RoundOptimiser {
		jebl.math.OrderEnumerator order_;
		jebl.math.OrthogonalLineFunction olf_;
		MultivariateFunction base_;

		public OrthogonalDirection(MultivariateFunction mf, jebl.math.OrderEnumerator.OEFactory orderFactory) {
			base_ = mf;
			olf_ = new jebl.math.OrthogonalLineFunction(base_);
	...
	private class OrthogonalHintsDirection implements RoundOptimiser {
		jebl.math.OrderEnumerator order_;
		jebl.math.OrthogonalLineFunction olf_;
		jebl.math.OrthogonalHints hints_;
		double[] store_ = new double[100];
		MultivariateFunction base_;
		public OrthogonalHintsDirection(MultivariateFunction mf, jebl.math.OrthogonalHints hints, jebl.math.OrderEnumerator.OEFactory orderFactory) {
			base_ = mf;
			olf_ = new jebl.math.OrthogonalLineFunction(base_);
