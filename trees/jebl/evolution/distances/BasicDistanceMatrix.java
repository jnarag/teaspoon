 ...
 * @version $Id: DistanceMatrix.java 842 2007-11-12 22:33:48Z twobeers $
 */
public class BasicDistanceMatrix implements DistanceMatrix {

    public BasicDistanceMatrix(Collection<jebl.evolution.taxa.Taxon> taxa, double[][] distances) {
        ...
        this.taxa = new ArrayList<jebl.evolution.taxa.Taxon>(taxa);
     ...
     */
    public List<jebl.evolution.taxa.Taxon> getTaxa() {
        return taxa;
    }
     ...
     * @return the distance
     */
    public double getDistance(jebl.evolution.taxa.Taxon taxonRow, jebl.evolution.taxa.Taxon taxonColumn) {
     ...
     */
    public DistanceMatrix getSubmatrix(Collection<jebl.evolution.taxa.Taxon> taxonSubset) {
        double[][] newDistances = new double[taxonSubset.size()][taxonSubset.size()];
        int i = 0;
        for (jebl.evolution.taxa.Taxon taxonRow : taxonSubset) {
            ...
            int j = 0;
            for (jebl.evolution.taxa.Taxon taxonColumn : taxonSubset) {
                int column = taxa.indexOf(taxonColumn);
                if (column == -1) {
    ...
    private final List<jebl.evolution.taxa.Taxon> taxa;
     ...
     */
    protected static double[][] buildDistancesMatrix(PairwiseDistanceCalculator pairwiseDistanceCalculator, int dimension, boolean useTwiceMaximumDistanceWhenPairwiseDistanceNotCalculatable, jebl.util.ProgressListener progress)
