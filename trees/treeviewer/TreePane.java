...
import virion.jam.controlpanels.ControlsProvider;
 ...
 * @version $Id: TreePane.java 953 2008-11-11 22:41:30Z stevensh $
 */
public class TreePane extends JComponent implements ControlsProvider, PainterListener, Printable {
    ...
    public jebl.evolution.trees.RootedTree getTree() {
        return tree;
    }

    public void setTree(jebl.evolution.trees.RootedTree tree, Collection<jebl.evolution.graphs.Node> selectedNodes) {
        ...
        for( Painter<?> p : pl ) {
            if( p instanceof jebl.gui.trees.treeviewer.painters.BasicLabelPainter) {
                ((jebl.gui.trees.treeviewer.painters.BasicLabelPainter)p).setTree(tree);
            }
        }
    ...
    public void setTreeLayout(jebl.gui.trees.treeviewer.treelayouts.TreeLayout treeLayout) {

        this.treeLayout = treeLayout;
        treeLayout.setTree(tree);
        treeLayout.addTreeLayoutListener(new jebl.gui.trees.treeviewer.treelayouts.TreeLayoutListener() {
...
//    }
    ...
    public void setSelectedNode(jebl.evolution.graphs.Node selectedNode) {
        selectedNodes.clear();
        selectedTaxa.clear();
    ...
    public void setSelectedClade(jebl.evolution.graphs.Node[] selectedNode) {
        selectedNodes.clear();
        selectedTaxa.clear();
        addSelectedClade(selectedNode, true);
    }

    public void setSelectedTaxa(jebl.evolution.graphs.Node selectedNode) {
        selectedNodes.clear();
        selectedTaxa.clear();
        addSelectedTaxa(selectedNode);
    }

    public void setSelectedNodes(Set<jebl.evolution.graphs.Node> nodes) {
        selectedNodes.clear();
        selectedTaxa.clear();
        addSelectedNodes(nodes);
    }

    private void addSelectedNodes(Set<jebl.evolution.graphs.Node> nodes) {
        selectedNodes.addAll(nodes);
        fireSelectionChanged();
        repaint();
    }

    private boolean canSelectNode(final jebl.evolution.graphs.Node selectedNode) {
        return selectedNode != null && isNodeVisible(selectedNode);
    }

    public void addSelectedNode(jebl.evolution.graphs.Node selectedNode, boolean add) {
    ...
    public void addSelectedNode(jebl.evolution.graphs.Node selectedNode) {
     ...
     */
    public void addSelectedClade(jebl.evolution.graphs.Node[] selectedNode, boolean add) {
    ...
    private void addSelectedChildClades(jebl.evolution.graphs.Node[] selectedNode, boolean add) {
    ...
    private void addSelectedChildClades(jebl.evolution.graphs.Node selectedNode, jebl.evolution.graphs.Node exclude, boolean add) {
        ...
        for (jebl.evolution.graphs.Node child : tree.getChildren(selectedNode)) {
            addSelectedChildClades(child, exclude, add);
        }
    }

    public void addSelectedTaxa(jebl.evolution.graphs.Node selectedNode) {
    ...
    private void addSelectedChildTaxa(jebl.evolution.graphs.Node selectedNode) {
    ...
    private boolean isNodeVisible(jebl.evolution.graphs.Node node) {
        return node.getAttribute(visibleAttributeName) == null;
    }

    private boolean isNodeCollapsed(jebl.evolution.graphs.Node node) {
         return node.getAttribute(clpsdName) != null;
    }

    private void setCladeVisisblty(final jebl.evolution.graphs.Node node, final boolean visible) {
        for( final jebl.evolution.graphs.Node child : tree.getChildren(node) ) {
            if( visible ) {
    ...
    private void expandContract(final jebl.evolution.graphs.Node selectedNode) {
    ...
    void toggleExpandContract(final jebl.evolution.graphs.Node selectedNode) {
        if( canSelectNode(selectedNode) ) {
            autoExpantion = false;
    ...
    private void setTreeAttributesForAutoExpansion() {
        for( jebl.evolution.graphs.Node node : nodesInOrder ) {
            node.removeAttribute(clpsdName);
            node.removeAttribute(visibleAttributeName);
        }

        if( autoExpantion ) {
            Set<jebl.evolution.graphs.Node> ignore = new HashSet<jebl.evolution.graphs.Node>();
            for( jebl.evolution.graphs.Node node : nodesInOrder ) {
    ...
    public void setTaxonLabelPainter(jebl.gui.trees.treeviewer.painters.BasicLabelPainter taxonLabelPainter) {
    ...
    public Painter<jebl.evolution.graphs.Node> getTaxonLabelPainter() {
        return taxonLabelPainter;
    }

    public void setNodeLabelPainter(Painter<jebl.evolution.graphs.Node> nodeLabelPainter) {
    ...
    public Painter<jebl.evolution.graphs.Node> getNodeLabelPainter() {
        return nodeLabelPainter;
    }

    public void setBranchLabelPainter(Painter<jebl.evolution.graphs.Node> branchLabelPainter) {
    ...
    public Painter<jebl.evolution.graphs.Node> getBranchLabelPainter() {
    ...
    jebl.evolution.graphs.Node[] getNodeAt(final Point point) {
        final Graphics2D g2 = (Graphics2D)getGraphics();
        jebl.evolution.graphs.Node[] result = new jebl.evolution.graphs.Node[2];
        ...
        jebl.evolution.graphs.Node rootNode = tree.getRootNode();
        if( ! hideNode(rootNode) && checkNodeIntersects(rootNode, point)) {
            result[0] = rootNode;
            return result;
        }
        List<jebl.evolution.graphs.Node> nodesInOrder = Utils.getNodes(tree, tree.getRootNode());
        ...
        jebl.evolution.graphs.Node[] externalNodes = tree.getExternalNodes().toArray(new jebl.evolution.graphs.Node[0]);
    ...
    private boolean checkNodeIntersects(jebl.evolution.graphs.Node node, Point point){
     ...
     * @return
     */
    Set<jebl.evolution.graphs.Node> getNodesAt(Graphics2D g2, Rectangle rect) {

        Set<jebl.evolution.graphs.Node> nodes = new HashSet<jebl.evolution.graphs.Node>();
...
//        }
        ...
        jebl.evolution.graphs.Node[] allNodes = tree.getNodes().toArray(new jebl.evolution.graphs.Node[0]);
...
//        System.err.println("tree draw " + (System.currentTimeMillis() - start) + "ms");
    ...
    private Point2D.Double nodeCoord(final jebl.evolution.graphs.Node node) {
    ...
    private void nodeMarker(Graphics2D g2, jebl.evolution.graphs.Node node, boolean alwaysDrawNodeMarkers) {
...
//            }
    ...
    private boolean hideNode(jebl.evolution.graphs.Node node) {
    ...
    public void drawTree(Graphics2D g2, boolean drawNodes, boolean clipOffscreenShapes, boolean drawOnlyVisibleElements, boolean drawAllNodeMarkers, double width, double height) {
        ...
        final Set<jebl.evolution.graphs.Node> externalNodes = tree.getExternalNodes();
        ...
        for (jebl.evolution.graphs.Node node : externalNodes) {
        ...
        final jebl.evolution.graphs.Node rootNode = tree.getRootNode();
        ...
        for(int nn = nodesInOrder.size()-1; nn >= 0; --nn) {
            final jebl.evolution.graphs.Node node = nodesInOrder.get(nn);
        ...
        if( ! preElementDrawCode ) {
            for( jebl.gui.trees.treeviewer.TreeDrawableElement e : treeElements ) {
...
//                    }
    ...
    private void calibrate(Graphics2D g2, double width, double height) {
        ...
        final Set<jebl.evolution.graphs.Node> externalNodes = tree.getExternalNodes();
        jebl.evolution.graphs.Node nodeWithLongestTaxon = null;
...
//                    + " y0 " + -bounds.getY() + "/" + yOffset + " x0 " + -bounds.getX() + "/" + xorigion);
        ...
        //treeBounds = null;
        {
            Set<jebl.evolution.graphs.Node> small = new HashSet<jebl.evolution.graphs.Node>();
            for (jebl.evolution.graphs.Node node : tree.getNodes()) {
                if( hideNode(node) ) continue;
...
//                }
        ...
        List<jebl.gui.trees.treeviewer.TreeDrawableElement> taxonLabels = new ArrayList<jebl.gui.trees.treeviewer.TreeDrawableElement>();
        if (taxonLabelPainter != null && taxonLabelPainter.isVisible()) {
            final double labelHeight = taxonLabelPainter.getPreferredHeight();
            ...
            for (jebl.evolution.graphs.Node node : externalNodes) {
                ...
                final jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel e =
                        new jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel(tree, node, just, labelBounds, taxonTransform, 10,
                                                         nodeWithLongestTaxon, (jebl.gui.trees.treeviewer.painters.BasicLabelPainter) taxonLabelPainter,
                        null);
        ...
        if (nodeLabelPainter != null && nodeLabelPainter.isVisible()) {
            ...
            for (jebl.evolution.graphs.Node node : tree.getNodes()) {
                ...
                if (labelPath != null) {
                    ...
                    final jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel e =
                        new jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel(tree, node, justification, labelBounds, labelTransform, 9,
                                                          null, ((jebl.gui.trees.treeviewer.painters.BasicLabelPainter) nodeLabelPainter), "node");
...
//                            ((BasicLabelPainter)branchLabelPainter).getFontSize() + " " + labelTransform);
                  ...
                  //  if( k == 0 ) continue;
                    ...
                    final jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel e =
                        new jebl.gui.trees.treeviewer.TreeDrawableElementNodeLabel(tree, node, Painter.Justification.CENTER, labelBounds, labelTransform, 8,
                                                          null, ((jebl.gui.trees.treeviewer.painters.BasicLabelPainter) branchLabelPainter), "branch");
        ...
        if( autoExpantion ) {
            ...
            for(int k = 0; k < treeElements.size(); ++k) {
                final jebl.gui.trees.treeviewer.TreeDrawableElement e = treeElements.get(k);
        ...
        jebl.gui.trees.treeviewer.TreeDrawableElement.setOverlappingVisiblitiy(treeElements, g2);
        jebl.gui.trees.treeviewer.TreeDrawableElement.setOverlappingVisiblitiy(taxonLabels, g2);
//        System.err.println("Clash " + (System.currentTimeMillis() - now));
        ...
        for(jebl.gui.trees.treeviewer.TreeDrawableElement element : taxonLabels){
...
//        System.err.println("Calibrate " + (System.currentTimeMillis() - start));
    ...
    jebl.gui.trees.treeviewer.treelayouts.TreeLayout getTreeLayout() {
        return treeLayout;
    }
    ...
    // Tree passed in
    private jebl.evolution.trees.RootedTree originalTree = null;
    //  Tree possibly transformed by the viewer
    private jebl.evolution.trees.RootedTree tree = null;
    private List<jebl.evolution.graphs.Node> nodesInOrder;

    private jebl.gui.trees.treeviewer.treelayouts.TreeLayout treeLayout = null;
    ...
    private Set<jebl.evolution.graphs.Node> selectedNodes = new HashSet<jebl.evolution.graphs.Node>();
    ...
    private jebl.gui.trees.treeviewer.painters.BasicLabelPainter taxonLabelPainter = null;
    private double taxonLabelWidth;
    private Painter<jebl.evolution.graphs.Node> nodeLabelPainter = null;
    private Painter<jebl.evolution.graphs.Node> branchLabelPainter = null;
    ...
    private Map<jebl.evolution.graphs.Node, AffineTransform> nodeLabelTransforms = new HashMap<jebl.evolution.graphs.Node, AffineTransform>();
    private Map<jebl.evolution.graphs.Node, Shape> nodeLabelBounds = new HashMap<jebl.evolution.graphs.Node, Shape>();
    private Map<jebl.evolution.graphs.Node, Painter.Justification> nodeLabelJustifications = new HashMap<jebl.evolution.graphs.Node, Painter.Justification>();

    private Map<jebl.evolution.graphs.Node, AffineTransform> branchLabelTransforms = new HashMap<jebl.evolution.graphs.Node, AffineTransform>();
    private Map<jebl.evolution.graphs.Node, Shape> branchLabelBounds = new HashMap<jebl.evolution.graphs.Node, Shape>();

    private List<jebl.gui.trees.treeviewer.TreeDrawableElement> treeElements = new ArrayList<jebl.gui.trees.treeviewer.TreeDrawableElement>();
