...
public class NeedlemanWunschAffine extends AlignAffine {
    ...
    public void doAlignment(String sequence1, String sequence2, int startType, int endType) {
        this.seq1 = sequence1;
        this.seq2 = sequence2;
        doAlignment(new jebl.evolution.align.Profile(0, sequence1), new jebl.evolution.align.Profile(0, sequence2), 0, 0, sequence1.length(), sequence2.length(), startType, endType);
...
//                s= sub.score [ sequence1.profile [i-1].characters[0]][ sequence2.profile [j-1].characters [0]];
                s = jebl.evolution.align.ProfileCharacter.score(sequence1.profile[offset1 + i - 1], sequence2.profile[offset2 + j - 1], sub);
                a = M[i - 1][j - 1] + s;
                b = Ix[i - 1][j - 1] + s;
                c = Iy[i - 1][j - 1] + s;
