 ...
 */
public class Coalescent implements jebl.math.MultivariateFunction {

	// PUBLIC STUFF

	public Coalescent(RootedTree tree, jebl.evolution.coalescent.DemographicFunction demographicFunction) {
		this(new Intervals(tree), demographicFunction);
	}

	public Coalescent(jebl.evolution.coalescent.IntervalList intervals, jebl.evolution.coalescent.DemographicFunction demographicFunction) {
	 ...
	 * given a demographic model.
	 */
	public static final double calculateLogLikelihood(jebl.evolution.coalescent.IntervalList intervals,
	                                                  jebl.evolution.coalescent.DemographicFunction demographicFunction) {
		...
		for (int i = 0, n = intervals.getIntervalCount(); i < n; i++) {
			...
			if (intervals.getIntervalType(i) == jebl.evolution.coalescent.IntervalList.IntervalType.COALESCENT) {

				logL += -Math.log(demographicFunction.getDemographic(finishTime)) -
									(jebl.math.Binomial.choose2(lineageCount)*intervalArea);

			} else { // SAMPLE or NOTHING

				logL += -(jebl.math.Binomial.choose2(lineageCount)*intervalArea);
			}
	 ...
	 */
	public static final double calculateAnalyticalLogLikelihood(jebl.evolution.coalescent.IntervalList intervals) {
	 ...
	 * out analytically. :-)
	 */
	private static final double getLambda(jebl.evolution.coalescent.IntervalList intervals) {
		double lambda = 0.0;
	...
	public jebl.math.OrthogonalHints getOrthogonalHints() {
		return null;
	}

	/** The demographic function. */
	private final jebl.evolution.coalescent.DemographicFunction demographicFunction;

	/** The intervals. */
	private final jebl.evolution.coalescent.IntervalList intervals;
}