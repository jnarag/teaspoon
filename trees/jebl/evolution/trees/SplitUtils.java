 ...
 *
 * @version $Id: SplitUtils.java 849 2007-12-06 00:10:14Z twobeers $
 *
...
public class SplitUtils {
	...
	public static SplitSystem getSplits(Tree tree) {
		return getSplits(new ArrayList<jebl.evolution.taxa.Taxon>(tree.getTaxa()), tree);
	}
	 ...
	 * @param tree
	 */
	public static SplitSystem getSplits(List<jebl.evolution.taxa.Taxon> taxa, Tree tree)
	{
		...
		int j = 0;
		for (jebl.evolution.graphs.Edge edge : tree.getInternalEdges()) {
	 ...
	 * @param split
	 */
	public static void getSplit(List<jebl.evolution.taxa.Taxon> taxa, Tree tree, jebl.evolution.graphs.Edge edge, boolean[] split) {
		...
		jebl.evolution.graphs.Node[] nodes = tree.getNodes(edge);
	...
	static void markNode(List<jebl.evolution.taxa.Taxon> taxa, Tree tree, jebl.evolution.graphs.Node node, jebl.evolution.graphs.Node parent, boolean[] split) {
		if (tree.isExternal(node)) {
			...
			for (jebl.evolution.graphs.Node child : tree.getAdjacencies(node)) {
                if (child != parent) {
