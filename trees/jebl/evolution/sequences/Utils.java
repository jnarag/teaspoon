...
public class Utils {
	...
	public static Sequence translate(final Sequence sequence, GeneticCode geneticCode) {
		return new jebl.evolution.sequences.BasicSequence(jebl.evolution.sequences.SequenceType.AMINO_ACID, sequence.getTaxon(), translate(sequence.getStates(), geneticCode));
	}
     ...
     *
     * Translation doesn't stop at stop codons; these are translated to {@link jebl.evolution.sequences.AminoAcids#STOP_STATE}.
     * If translating from {@link NucleotideState} and the number
    ...
    private static String reverseComplement(final String nucleotideSequence, boolean removeGaps) {
        ...
        Sequence seq = new jebl.evolution.sequences.BasicSequence(jebl.evolution.sequences.SequenceType.NUCLEOTIDE, jebl.evolution.taxa.Taxon.getTaxon("x"), nucleotideSequence);
        if( removeGaps ) {
            seq = new jebl.evolution.sequences.GaplessSequence(seq);
        }
        int length=seq.getLength();
        StringBuilder results =new StringBuilder();
        for (int i =  length-1; i >=0; i--) {
            State state= seq.getState(i);
            NucleotideState complementaryState = jebl.evolution.sequences.Nucleotides.COMPLEMENTARY_STATES[state.getIndex()];
            if (predominantlyRNA && complementaryState.equals(jebl.evolution.sequences.Nucleotides.T_STATE)) {
                results.append('U');
            }
            else {
...
/*
    ...
    public static String translateCharSequence(final CharSequence nucleotideSequence, GeneticCode geneticCode) {
        ...
        seq = new jebl.evolution.sequences.BasicSequence(jebl.evolution.sequences.SequenceType.AMINO_ACID, jebl.evolution.taxa.Taxon.getTaxon("x"), states);
        return seq.getString();
    }
    ...
    public static NucleotideState[] complement(final NucleotideState[] sequence) {
        ...
        for (int i = 0; i < sequence.length; i++) {
            complemented[i] = jebl.evolution.sequences.Nucleotides.COMPLEMENTARY_STATES[sequence[i].getIndex()];
        }
        return complemented;
    }
    ...
    public static NucleotideState[] reverseComplement(final NucleotideState[] sequence) {
        ...
        for (int i = 0; i < sequence.length; i++) {
            reverseComplemented[i] = jebl.evolution.sequences.Nucleotides.COMPLEMENTARY_STATES[sequence[sequence.length - i - 1].getIndex()];
        }
     ...
     */
    public static jebl.evolution.sequences.SequenceType guessSequenceType(final CharSequence seq) {
        ...
        final int canonicalStateCount = jebl.evolution.sequences.Nucleotides.getCanonicalStateCount();
        ...
        for(int k = 0; (k < seqLen) && (onlyValidNucleotides || onlyValidAminoAcids); ++k) {
            ...
            final NucleotideState nucState = jebl.evolution.sequences.Nucleotides.getState(c);
            final boolean isNucState = nucState != null;
            final boolean isAminoState = jebl.evolution.sequences.AminoAcids.getState(c) != null;
            ...
            if (onlyValidNucleotides) {
                ...
                if (nucState.getIndex() < canonicalStateCount) {
                    ...
                    if (nucState == jebl.evolution.sequences.Nucleotides.GAP_STATE) {
                        --sequenceLength;
                    } else if( nucState == jebl.evolution.sequences.Nucleotides.N_STATE ) {
                        ++undeterminedStates;
                    }
                }
            }
        ...
        jebl.evolution.sequences.SequenceType result;
        if (onlyValidNucleotides) {  // only nucleotide states
            ...
            if( sequenceLength >= 100 ) {
                result = jebl.evolution.sequences.SequenceType.NUCLEOTIDE;
            } else {
                ...
                // in AminoAcids.java.
                result = nucStates >= sequenceLength * threshold ? jebl.evolution.sequences.SequenceType.NUCLEOTIDE : jebl.evolution.sequences.SequenceType.AMINO_ACID;
            }
        } else if (onlyValidAminoAcids) {
            result = jebl.evolution.sequences.SequenceType.AMINO_ACID;
        } else {
            result = null;
        }
        return result;
	 ...
	 */
	public static State[] cleanSequence(final CharSequence seq, final jebl.evolution.sequences.SequenceType type) {
		int count = 0;
