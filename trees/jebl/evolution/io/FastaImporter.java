...
public class FastaImporter implements SequenceImporter, ImmediateSequenceImporter {
     ...
     */
    private List<jebl.evolution.sequences.Sequence> read(ImmediateSequenceImporter.Callback callback,
                                jebl.util.ProgressListener progressListener)
            throws IOException, ImportException
    {
        final List<jebl.evolution.sequences.Sequence> sequences = (callback == null) ? new ArrayList<jebl.evolution.sequences.Sequence>() : null;
...
//                System.out.println("after reading sequence " + (s_runtime.totalMemory() - s_runtime.freeMemory())/1000 + " / " + s_runtime.totalMemory()/1000);
                ...
                final jebl.evolution.taxa.Taxon taxon = jebl.evolution.taxa.Taxon.getTaxon(name);
                ...
                SequenceType type = ( sequenceType != null ) ? sequenceType : jebl.evolution.sequences.Utils.guessSequenceType(seq);
                ...
                if( type == null ) {
                    ...
                    switch (illegalCharacterPolicyForThisImport.get()) {
                        case strip:
                            ...
                            type = jebl.evolution.sequences.Utils.guessSequenceType(seq);
                            if (type==null) {
     ...
     * @throws ImportException
     */
    public final List<jebl.evolution.sequences.Sequence> importSequences() throws IOException, ImportException {
        return read(null, jebl.util.ProgressListener.EMPTY);
    }

    public void importSequences(Callback callback, jebl.util.ProgressListener progressListener) throws IOException, ImportException {
