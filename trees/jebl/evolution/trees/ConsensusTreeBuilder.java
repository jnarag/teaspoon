...
public abstract class ConsensusTreeBuilder<T extends Tree> implements TreeBuilder<T> {
    ...
    protected final List<jebl.evolution.taxa.Taxon> taxons;
     ...
     * @param supportAttributeName name of attribute (see {@link Tree#getAttribute(String)}) describing a tree's amount of support
    ...
    ConsensusTreeBuilder(Tree[] trees, String supportAttributeName, boolean supportInPercent) throws IllegalArgumentException {
        ...
        this.taxons = Collections.unmodifiableList(new ArrayList<jebl.evolution.taxa.Taxon>(trees[0].getTaxa()));
    ...
    public void addProgressListener(jebl.util.ProgressListener listener) {
     ...
     */
    public void removeProgressListener(jebl.util.ProgressListener listener) {
        synchronized(listeners) {
    ...
    protected boolean fireSetProgress(double fractionCompleted) {
        ...
        List<jebl.util.ProgressListener> listenersCopy;
        synchronized(listeners) { // create a copy because we don't want to hold the lock while calling the listeners
            listenersCopy = new ArrayList<jebl.util.ProgressListener>(listeners);
        }
        for (jebl.util.ProgressListener listener : listenersCopy) {
    ...
    private final List<jebl.util.ProgressListener> listeners = new ArrayList<jebl.util.ProgressListener>();
}