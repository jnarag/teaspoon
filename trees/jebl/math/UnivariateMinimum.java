...
public class UnivariateMinimum
{
	...
	{
		double tol = jebl.math.MachineAccuracy.EPSILON;
	...
	private double[] bracketize(double min, double a, double max, UnivariateFunction f)
	{
		...
		while (fb > fc)
		{
			...
			if (q == r)
			{
										q += jebl.math.MachineAccuracy.EPSILON;
			}
	...
	private double minin(double a, double b, double fa , double fb, UnivariateFunction f, double tol)
	{
		...
		if (a == b)
		{
			...
			f2minx = jebl.math.NumericalDerivative.secondDerivative(f, minx);
		...
		{
			m = (a + b)*0.5;
			double tol_act = jebl.math.MachineAccuracy.SQRT_EPSILON + tol; // Absolute tolerance
